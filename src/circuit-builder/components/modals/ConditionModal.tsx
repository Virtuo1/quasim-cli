...
import { describeExpr, exprRegisters, getExprAtPath, replaceExprKind, updateExprAtPath, validateConditionExpression } from "../../utils/conditions";
...
export function ConditionModal({
  ...
  useEffect(() => {
    ...
    // Keep panning responsive even when the pointer leaves the tree viewport.
        ...
        <div style={splitPaneStyle}>
          <div style={treePanelStyle}>
            ...
            <div style={treeViewportStyle}>
              ...
              >
                <rect
                  data-role="tree-background"
                  x={0}
                  y={0}
                  width="100%"
                  height="100%"
                  fill={UI_COLORS.panelBg}
                />
          ...
          <div style={inspectorPanelStyle}>
            ...
            <div style={inspectorContentStyle}>
              <SummaryCard title="Selected Node" value={describeExpr(selectedExpr)} mono />
...
function ExprTreeNodeView({ node, selected, onSelect }: TreeNodeViewProps) {
  const meta = getExprKindMeta(node.expr.kind);
  const nodeStroke = selected ? UI_COLORS.amber500 : UI_COLORS.borderMid;
  const nodeFill = getExprNodeFill(node.expr.kind);
...
function getExprNodeFill(kind: Expr["kind"]) {
  switch (kind) {
    case "reg":
...
function describeLeafNodeValue(expr: Expr): string {
  switch (expr.kind) {
    case "reg":
...
// The tree is auto-laid out from subtree widths so the user never drags nodes into place.
...
function layoutNode(expr: Expr, path: string, originX: number, originY: number): TreeLayout {
  ...
  return {
    ...
    nodes: [{ path, expr, x: nodeX, y: nodeY, ...nodeSize }],
    edges: [],
  };
}
